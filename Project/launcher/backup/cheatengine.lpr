...
begin
  ...
  if launch32bit then
    MessageBox(0,'A fucking retard thought that running this on a 32bit processor is a good idea','Launch error',0)
  else
  begin
    asm
      push ecx
    ...
    if (cpuid7ebx and (1 shl 5))>0 then
      exename:=basename+'-x64-SSE4-AVX2.exe'
    else
      exename:=basename+'-x64.exe';
  end;
  ...
  begin
    s:=exename;
    MessageBoxW(0, pwidechar(exename+' could not be found. Please disable/uninstall your anti virus and reinstall 999 Engine to fix this'),'Launch error',MB_OK or MB_ICONERROR);
  end;
